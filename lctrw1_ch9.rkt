...
(define positions (positions-from-file "../krr-test/fen_lctrw1_ch9.fen"))
(define movestrings (file->lines "solutions_lctrw1_ch9.txt"))
...
(for ([pos positions] [movestr movestrings] [index (range 1 (+ 1 (length positions)))])
  (let* ([move (movestring->move pos movestr)]
    ...
    (displayln (format "~a: ~a" index
                       (move-evaluations-cmp-string move calculated-moves)))))


;(move-in-evaluations-string move calculated-moves)))))
